...
def start():
    ...
    initial_board, solution_board,pre_filled_cells = generate_puzzle()
    ...
    while True:
        ...
        should_quit = handle_command(parsed, board,pre_filled_cells)
        if should_quit:
            break

        if is_game_completed(board,solution_board):
            print_board(board)
